# Microsoft「MAI」3モデルがFoundryに揃った｜自社スタックを開発者向けに切り出した発表

## はじめに

本記事では、**2026年4月2日**に **Microsoft AI**（署名: Mustafa Suleyman）が公表した **MAI-Transcribe-1 / MAI-Voice-1 / MAI-Image-2** の **Microsoft Foundry** 提供について、**[公式ブログ](https://microsoft.ai/news/today-were-announcing-3-new-world-class-mai-models-available-in-foundry/)** および **Microsoft Learn** に書いてある範囲を中心に、実務で使う目線で要点を整理します。
...
- **事実:**  
  - **MAI-Transcribe-1**: 音声→テキスト。**FLEURS** を引用し、**Microsoft 製品利用の多い上位25言語**を軸に説明。バッチ文字起こしは **既存 Azure Fast 比 2.5 倍**などと記載。  
  - **MAI-Voice-1**: 音声合成。**数秒の音声**からカスタム声、**1秒で約60秒分の音声**などと記載。**Copilot Audio Expressions** / **Copilot Podcasts** への導線あり。  
  - **MAI-Image-2**: 画像生成。**Arena.ai** でモデルファミリー **トップ3** などと引用。Foundry / Copilot では **少なくとも2倍速**などの主張。**Bing**・**PowerPoint** への展開にも言及。  
...
- **補足:** 価格は **リージョン・通貨・課金単位**で変わりうる。**脚注の競合比較**（Whisper-large-v3、Gemini 3.1 Flash 等）は**評価条件とモデル版**まで含めて読まないとズレやすい。
...
*企画・下調べメモ: `reports/note-briefs/2026-04-04-microsoft-mai-foundry-models.md`*
